...
const CONFIG = {
    ...
    // Stroke-count / coverage penalties (proportional, not flat — see matchDirectionVectors)
    STROKE_COUNT_PENALTY_WEIGHT: 0.35,
    COVERAGE_PENALTY_WEIGHT: 0.35,

    // Ambiguity detection: how close top-1 and top-2 blended scores can be
    // before we stop reporting top-1's raw confidence at face value
    AMBIGUITY_MARGIN: 0.05,

    // Similarity falloff sharpness — higher values separate genuinely
    // different shapes faster instead of clustering everything into a narrow
    // high-similarity band. Tuned empirically against the full dataset via
    // diagnostic-tools/analyze.js (default 1.5/1.0 clustered 26 character
    // pairs within 0.05 of each other; these values bring that down to 13,
    // all of which are genuine hiragana/katakana lookalikes).
...
function pointsToDirectionVectors(points) {
    ...
    for (let i = 1; i < points.length; i++) {
        const dx = points[i].x - points[i-1].x;
        const dy = points[i].y - points[i-1].y;
        // A zero-length segment (duplicate consecutive points, or a
        // degenerate single-point stroke) has no real direction —
        // atan2(0,0) would silently produce a spurious "0" (→) vector
        // that pollutes edit-distance matching. Skip it instead.
...
function matchDirectionVectors(userVectors, refVectors) {
    ...
    // Proportional penalties: a 1-stroke difference on a 2-stroke character should
    // hurt far more than a 1-stroke difference on an 11-stroke character. Flat
    // per-stroke penalties let unrelated low-stroke-count characters cluster
    // together near the top of the baseline similarity band; scaling by the
    // fraction of strokes mismatched/uncovered fixes that.
...
function matchAgainstDataset(features) {
    ...
    // Ambiguity-margin confidence adjustment: if the top-2 candidates are a
    // near-tie, reporting top-1's raw score gives false confidence — a 0.86
    // score means very different things depending on whether 2nd place is at
    // 0.60 or 0.85. Scale the *displayed* confidence down as the margin
    // shrinks, without touching the underlying score used for ranking.
    if (results.length > 0) {
        const margin = results.length > 1 ? results[0].score - results[1].score : 1;
        const isAmbiguous = margin < CONFIG.AMBIGUITY_MARGIN;
        const marginFactor = Math.min(1, margin / CONFIG.AMBIGUITY_MARGIN);
        results[0].margin = margin;
        results[0].isAmbiguous = isAmbiguous;
        // At the ambiguity threshold or beyond, show the raw score. Below it,
        // scale down toward (but not below) half the raw score, so a dead-tie
        // (margin ~0) reads as roughly half confidence instead of a
        // deceptively high one.
        results[0].displayConfidence = isAmbiguous
            ? results[0].score * (0.5 + 0.5 * marginFactor)
            : results[0].score;
    }
...
function displayResults(results, userStrokeCount, allFeatures) {
    ...
    const shownConfidence = best.displayConfidence !== undefined ? best.displayConfidence : best.score;
    confidenceEl.textContent = Math.round(shownConfidence * 100) + '%';
    if (best.isAmbiguous && results.length > 1) {
        confidenceEl.title = `Close call — "${results[1].char}" scored nearly as well (${Math.round(results[1].score * 100)}%)`;
        confidenceEl.classList.add('ambiguous');
    } else {
        confidenceEl.removeAttribute('title');
        confidenceEl.classList.remove('ambiguous');
    }
...
// ==================== DEV: DATASET SELF-TEST ====================
// Feeds every character's own refStrokes back into the real matching
// pipeline and checks it recognizes itself as the #1 result. Also flags
// pairs whose top-2 scores land within CONFIG.AMBIGUITY_MARGIN of each
// other. Catches regressions in data.js or the matching algorithm without
// needing the external diagnostic-tools/analyze.js script. Runs only when
// explicitly requested (?debug=1 in the URL) since it's O(n^2) over the
// full dataset and not needed for normal use.
function selfTestDataset() {
    console.log(`[selfTest] Running dataset self-test over ${DATASET.length} characters...`);
    let failures = [];
    let ambiguous = [];

    DATASET.forEach(entry => {
        const bbox = computeBoundingBox(entry.refStrokes);
        if (!bbox) { failures.push({ char: entry.char, reason: 'bounding box too small' }); return; }

        const resampled = entry.refStrokes.map(s => resampleStroke(s, CONFIG.RESAMPLE_POINTS));
        const userVectors = resampled.map(pointsToDirectionVectors);
        const userAnchors = computePositionAnchors(resampled);
        const userGrids = computeGridOccupancy(resampled);
        const userCurv = computeCurvatureSignatures(resampled);
        const userProps = computeStrokeProportions(resampled);
        const userPixel = computePixelMatrix(entry.refStrokes, bbox);

        const results = matchAgainstDataset({
            userVectors, userAnchors, userGrids, userCurv, userProps, userPixel,
            userStrokeCount: entry.refStrokes.length
        });

        if (results[0].char !== entry.char) {
            const selfRank = results.findIndex(r => r.char === entry.char);
            failures.push({
                char: entry.char, name: entry.name,
                beatenBy: results[0].char, beatenByScore: results[0].score.toFixed(3),
                selfScore: results[selfRank] ? results[selfRank].score.toFixed(3) : 'n/a'
            });
        } else if (results.length > 1 && results[0].isAmbiguous) {
            ambiguous.push({
                char: entry.char, name: entry.name,
                score: results[0].score.toFixed(3),
                runnerUp: results[1].char, runnerUpScore: results[1].score.toFixed(3)
            });
        }
    });

    if (failures.length === 0) {
        console.log('[selfTest] ✓ Every character recognizes itself as the top match.');
    } else {
        console.warn(`[selfTest] ✗ ${failures.length} character(s) don't self-match:`, failures);
    }
    if (ambiguous.length > 0) {
        console.warn(`[selfTest] ${ambiguous.length} character(s) are ambiguous (within ${CONFIG.AMBIGUITY_MARGIN} of runner-up):`, ambiguous);
    }
